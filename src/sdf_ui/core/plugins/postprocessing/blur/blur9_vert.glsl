...
void main() {
    ...
    for (int i = 1; i < 5; i++) {
        col += imageLoad(origTex, abs_i2(texelPos - ivec2(0.0, offset[i]))) * weight[i];

        if (texelPos.y + i >= size.y - 1) {
            factor = -1;
        }

        col += imageLoad(origTex, texelPos + ivec2(0.0, factor * offset[i])) * weight[i];
    }
